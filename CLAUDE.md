...
The solution has ten projects plus two samples:

- **pg_protoexport.core** - Library with parsing and domain models. No dependency on CLI or export formats. Also exposes `ProtocolStateProjector` / `ProtocolStateSnapshot` under `Sequence/` for folding a packet stream into per-message observable state (connection state, transaction status, prepared statements, portals, server parameters, backend PID) used by the HTML exporter. Hosts the live-capture primitive `LiveCaptureSession` (and `ILiveCaptureSessionFactory`, `LiveCaptureOptions`, `PcapDevicePicker`) alongside the read-side `PcapService`. Defines `IPcapExporter` (the format-agnostic export contract).
